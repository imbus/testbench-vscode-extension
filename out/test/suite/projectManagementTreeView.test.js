...
const projectManagementTreeItem_1 = require("../../views/projectManagement/projectManagementTreeItem");
const projectDataService_1 = require("../../views/projectManagement/projectDataService");
const mockDataFactory_1 = require("../utils/mockDataFactory");
const treeViewStateTypes_1 = require("../../views/common/treeViewStateTypes");
const constants_1 = require("../../constants");
suite("ProjectManagementTreeDataProvider Tests", () => {
    let testEnv;
    let provider;
    let mockProjectDataService;
    ...
    test("should return TOV and Cycle items as children of a Project", async () => {
        // Arrange
        const projectItem = new projectManagementTreeItem_1.ProjectManagementTreeItem("Project Gamma", constants_1.TreeItemContextValues.PROJECT, 1, // Collapsed
        (0, mockDataFactory_1.createMockProject)({ key: "proj_3", tovsCount: 1 }), testEnv.mockContext, testEnv.logger, testEnv.iconService, null);
        const mockTree = (0, mockDataFactory_1.createMockTreeNode)({
            key: "proj_3_root",
            children: [
                (0, mockDataFactory_1.createMockTreeNode)({ key: "tov_1", name: "TOV 1", nodeType: "Version" }),
                (0, mockDataFactory_1.createMockTreeNode)({ key: "cycle_1", name: "Cycle 1", nodeType: "Cycle" })
            ]
        });
        ...
        assert.ok(mockProjectDataService.getProjectTree.calledOnceWith("proj_3"), "getProjectTree should be called with the project key");
    });
    test("should return empty array when fetching children fails", async () => {
        // Arrange
        const projectItem = new projectManagementTreeItem_1.ProjectManagementTreeItem("Project Delta", constants_1.TreeItemContextValues.PROJECT, 1, (0, mockDataFactory_1.createMockProject)({ key: "proj_4" }), testEnv.mockContext, testEnv.logger, testEnv.iconService, null);
        ...
        assert.ok(testEnv.logger.error.calledWith(sinon.match.string, childFetchError), "Logger should have recorded the child fetch error");
    });
    test("should correctly set and reset a custom root", async () => {
        // Arrange
        const projectItem = new projectManagementTreeItem_1.ProjectManagementTreeItem("Project Epsilon", constants_1.TreeItemContextValues.PROJECT, 1, (0, mockDataFactory_1.createMockProject)({ key: "proj_5" }), testEnv.mockContext, testEnv.logger, testEnv.iconService, null);
        // Act & Assert (Set Root)
        ...
        assert.strictEqual(provider.getCurrentCustomRoot()?.label, "Project Epsilon", "The correct item should be set as custom root");
    ...
    test("should set collapsibleState based on child counts", () => {
        // Arrange
        const projectWithChildren = (0, mockDataFactory_1.createMockProject)({ tovsCount: 1, cyclesCount: 1 });
        const projectWithoutChildren = (0, mockDataFactory_1.createMockProject)({ tovsCount: 0, cyclesCount: 0 });
        // Act
        ...
        assert.strictEqual(itemWithoutChildren?.collapsibleState, 0, "Project without children should not be collapsible"); // 0 is None
    });
    test("should correctly fetch children in a multi-level hierarchy", async () => {
        // Arrange
        const mockCycleNode = (0, mockDataFactory_1.createMockTreeNode)({ key: "cycle_1", name: "Cycle 1.1", nodeType: "Cycle" });
        const mockTovNode = (0, mockDataFactory_1.createMockTreeNode)({
            key: "tov_1",
            name: "TOV 1",
            nodeType: "Version",
            children: [mockCycleNode]
        });
        const mockProjectTree = (0, mockDataFactory_1.createMockTreeNode)({
            key: "proj_root_1",
            children: [mockTovNode]
        });
        mockProjectDataService.getProjectTree.resolves(mockProjectTree);
        const projectItem = new projectManagementTreeItem_1.ProjectManagementTreeItem("Project with Depth", constants_1.TreeItemContextValues.PROJECT, 1, (0, mockDataFactory_1.createMockProject)({ key: "proj_deep" }), testEnv.mockContext, testEnv.logger, testEnv.iconService, null);
        // Act
    ...
    test("should fetch correct children when getChildren is called on a custom root", async () => {
        // Arrange
        const mockCycleNode1 = (0, mockDataFactory_1.createMockTreeNode)({ key: "cycle_2", name: "Cycle 2.1", nodeType: "Cycle" });
        const mockCycleNode2 = (0, mockDataFactory_1.createMockTreeNode)({ key: "cycle_3", name: "Cycle 2.2", nodeType: "Cycle" });
        const mockTovNode = (0, mockDataFactory_1.createMockTreeNode)({
            key: "tov_2",
            name: "Complex TOV",
            nodeType: "Version",
        ...
        const mockProjectTree = (0, mockDataFactory_1.createMockTreeNode)({ key: "proj_root_2", children: [mockTovNode] });
        mockProjectDataService.getProjectTree.resolves(mockProjectTree);
        const rootProjectItem = new projectManagementTreeItem_1.ProjectManagementTreeItem("Project for Custom Root", constants_1.TreeItemContextValues.PROJECT, 1, (0, mockDataFactory_1.createMockProject)({ key: "proj_custom_root" }), testEnv.mockContext, testEnv.logger, testEnv.iconService, null);
    ...
    suite("getProjectAndTovNamesFromProjectTreeItem Tests", () => {
        ...
        setup(() => {
            ...
            const projectData = (0, mockDataFactory_1.createMockProject)({ key: "p1", name: "MyProject" });
            projectItem = new projectManagementTreeItem_1.ProjectManagementTreeItem("MyProject", constants_1.TreeItemContextValues.PROJECT, 1, projectData, testEnv.mockContext, testEnv.logger, testEnv.iconService, null);
            const tovData = (0, mockDataFactory_1.createMockTreeNode)({ key: "t1", name: "MyTOV" });
            tovItem = new projectManagementTreeItem_1.ProjectManagementTreeItem("MyTOV", constants_1.TreeItemContextValues.VERSION, 1, tovData, testEnv.mockContext, testEnv.logger, testEnv.iconService, projectItem);
            projectItem.children = [tovItem];
            const cycleData = (0, mockDataFactory_1.createMockTreeNode)({ key: "c1", name: "MyCycle" });
            cycleItem = new projectManagementTreeItem_1.ProjectManagementTreeItem("MyCycle", constants_1.TreeItemContextValues.CYCLE, 0, cycleData, testEnv.mockContext, testEnv.logger, testEnv.iconService, tovItem);
    ...
    suite("Custom Root State Management", () => {
        test("should correctly save the state when makeRoot is called", () => {
            // Arrange
            const tovItem = new projectManagementTreeItem_1.ProjectManagementTreeItem("TOV for Serialization", constants_1.TreeItemContextValues.VERSION, 1, (0, mockDataFactory_1.createMockTreeNode)({ key: "tov_serialize" }), testEnv.mockContext, testEnv.logger, testEnv.iconService, null);
            const updateStub = testEnv.mockContext.workspaceState.update;
            ...
            assert.strictEqual(storageKey, "testbenchExtension.customRoot.projectTree", "State saved with incorrect key");
        ...
        test("should restore a pending custom root after a refresh", async () => {
            ...
            const mockProjects = [(0, mockDataFactory_1.createMockProject)({ key: "proj_restore", name: "Restored Project" })];
            mockProjectDataService.getProjectsList.resolves(mockProjects);
