...
export let projectManagementTreeDataProvider: projectManagementTreeView.ProjectManagementTreeDataProvider | null =
  null;
...
export function setConnection(
  newConnection: testBenchConnection.PlayServerConnection | null,
): void {
  connection = newConnection;
}

/** Global login webview provider instance. */
export let loginWebViewProvider: loginWebView.LoginWebViewProvider | null =
  null;
 ...
 */
export function safeCommandHandler(
 ...
 */
export async function promptForWorkspaceLocation(): Promise<
  string | undefined
> {
...
export function initializeTreeViews(): void {
  ...
  projectManagementTreeDataProvider =
    new projectManagementTreeView.ProjectManagementTreeDataProvider(null!);
  ...
  testElementsTreeDataProvider =
    new testElementsTreeView.TestElementsTreeDataProvider();
...
function registerExtensionCommands(context: vscode.ExtensionContext): void {
  ...
  registerSafeCommand(
    context,
    `${baseKeyOfExtension}.setWorkspaceLocation`,
    async () => {
      ...
      if (newWorkspaceLocation) {
        ...
        vscode.window.showInformationMessage(
          `Workspace location set to: ${newWorkspaceLocation}`,
        );
  ...
  registerSafeCommand(
    context,
    allExtensionCommands.showExtensionSettings.command,
    async () => {
  ...
  registerSafeCommand(
    ...
    async () => {
      ...
      if (
        ...
        if (
          await testBenchConnection?.performLogin(
            context,
            baseKeyOfExtension,
            false,
            true,
          ...
          await vscode.commands.executeCommand(
            allExtensionCommands.selectAndLoadProject.command,
          );
        }
      } else {
  ...
  registerSafeCommand(context, allExtensionCommands.login.command, async () => {
    ...
    if (isLoginProcessAlreadyRunning) {
      ...
      setTimeout(() => {
        ...
        logger.trace(
          "isLoginProcessAlreadyRunning flag reset to false after 5 seconds.",
        );
      }, 5000);
      return;
    }
    ...
    try {
      const performLoginResult = await testBenchConnection.performLogin(
        context,
        baseKeyOfExtension,
      );
      // If login was successful, display project selection dialog and the project management tree view.
      ...
      isLoginProcessAlreadyRunning = false;
      logger.trace(
        "isLoginProcessAlreadyRunning flag is reset to false after login attempt.",
      );
    }
  ...
  registerSafeCommand(
    context,
    allExtensionCommands.logout.command,
    async () => {
      logger.debug("Logout command called.");
      if (!connection) {
        vscode.window.showErrorMessage(
          "No connection available. Please log in first.",
        );
  ...
  registerSafeCommand(
    ...
    async (item: projectManagementTreeView.ProjectManagementTreeItem) => {
      ...
      if (!connection) {
        ...
        logger.warn(
          "generateTestCasesForCycle command called without connection.",
        );
        return;
      }
      if (!projectManagementTreeDataProvider) {
        ...
        logger.warn(
          "generateTestCasesForCycle command called without project data provider.",
        );
        return;
      }
      ...
      if (config.get<boolean>("clearWorkingDirectoryBeforeTestGeneration")) {
        await vscode.commands.executeCommand(
          allExtensionCommands.clearWorkspaceFolder.command,
        );
      }
  ...
  registerSafeCommand(
    ...
    async (treeItem: projectManagementTreeView.ProjectManagementTreeItem) => {
      logger.debug(
        "Generate Test Cases For Test Theme or Test Case Set command called.",
      );
      if (!connection) {
        vscode.window.showErrorMessage(
          "No connection available. Please log in first.",
        ...
        logger.warn(
          "generateTestCasesForTestThemeOrTestCaseSet command called without connection.",
        );
        return;
      }
      // Optionally clear the working directory before test generation.
      ...
      logger.trace(
        "End of Generate Test Cases For Test Theme or Test Case Set command.",
      );
    },
  );
  ...
  registerSafeCommand(
    ...
    async () => {
      ...
      const projectList: testBenchTypes.Project[] | null =
        await connection.getProjectsList();
      if (!projectList) {
        logger.warn("No projects found for selectAndLoadProject command.");
        return;
      }
      const selectedProjectKey: string | null =
        await connection.getProjectKeyFromProjectListQuickPickSelection(
          projectList,
        );
      ...
      projectManagementTreeDataProvider =
        new projectManagementTreeView.ProjectManagementTreeDataProvider(
          selectedProjectKey,
        );
      projectTreeView = vscode.window.createTreeView("projectManagementTree", {
        treeDataProvider: projectManagementTreeDataProvider,
      ...
      [projectManagementTreeDataProvider] =
        await projectManagementTreeView.initializeTreeViews(
          context,
          connection,
          selectedProjectKey,
        );
      logger.trace(
        `Project with key ${selectedProjectKey} loaded into project management tree view.`,
      );
    },
  );
  ...
  registerSafeCommand(
    context,
    allExtensionCommands.toggleTestThemeTreeViewVisibility.command,
    async () => {
  ...
  registerSafeCommand(
    ...
    async () => {
      logger.debug(
        "Read RF Test Results And Create Report With Results command called.",
      );
      if (!connection) {
        vscode.window.showErrorMessage(
          "No connection available. Please log in first.",
        ...
        logger.warn(
          "readRFTestResultsAndCreateReportWithResults command called without connection.",
        );
        return;
      }
      ...
      logger.trace(
        "End of Read RF Test Results And Create Report With Results command.",
      );
    },
  );
  ...
  registerSafeCommand(
    context,
    allExtensionCommands.uploadTestResultsToTestbench.command,
    async () => {
      ...
      if (!connection) {
        ...
        logger.warn(
          "uploadTestResultsToTestbench command called without connection.",
        );
        return;
      }
      if (
        ...
        logger.warn(
          "uploadTestResultsToTestbench command called without a selected project.",
        );
        return;
      }
  ...
  registerSafeCommand(
    context,
    allExtensionCommands.readAndUploadTestResultsToTestbench.command,
    async () => {
  ...
  registerSafeCommand(
    context,
    allExtensionCommands.refreshProjectTreeView.command,
    async () => {
  ...
  registerSafeCommand(
    context,
    allExtensionCommands.refreshTestThemeTreeView.command,
    async () => {
      logger.debug("Refresh Test Theme Tree command called.");
      projectManagementTreeDataProvider?.testThemeDataProvider.refresh();
      const cycleElement =
        projectManagementTreeDataProvider?.testThemeDataProvider
          ?.rootElements[0]?.parent ?? undefined;
      if (cycleElement && cycleElement.contextValue === "Cycle") {
        ...
        const children: projectManagementTreeView.ProjectManagementTreeItem[] =
          (await projectManagementTreeDataProvider?.getChildrenOfCycle(
            cycleElement,
          )) ?? [];
        projectManagementTreeDataProvider?.testThemeDataProvider?.setRoots(
          children,
    ...
    (treeItem: projectManagementTreeView.ProjectManagementTreeItem) => {
      ...
      if (projectManagementTreeDataProvider) {
        ...
        if (
          ...
          ["Project", "Version", "Cycle"].includes(treeItem.contextValue)
        ) {
          projectManagementTreeDataProvider.makeRoot(treeItem);
        } else {
          projectManagementTreeDataProvider.testThemeDataProvider.makeRoot(
            treeItem,
          );
        }
      }
  ...
  registerSafeCommand(
    ...
    async (treeItem: projectManagementTreeView.ProjectManagementTreeItem) => {
      logger.debug(
        "Display Interactions For Selected TOV command called for tree item:",
        treeItem,
      );
  ...
  registerSafeCommand(
    ...
    async (treeItem: testElementsTreeView.TestElementItem) => {
      if (!treeItem || !treeItem.element) {
        logger.trace(
          "Invalid tree item or element in goToTestElementFile command.",
        );
        return;
      }
      ...
      const baseTargetPath = path.join(
        workspaceRootPath,
        ...testElement.hierarchicalName.split("/"),
      );
      try {
        switch (testElement.elementType) {
          case "Subdivision":
            await testElementsTreeView.handleSubdivision(
              testElement,
              baseTargetPath,
            );
            break;
          case "Interaction":
            await testElementsTreeView.handleInteraction(
              testElement,
              workspaceRootPath,
            );
            break;
        ...
        vscode.window.showErrorMessage(
          "Error in goToFile command: " + error.message,
        );
  ...
  vscode.commands.executeCommand(
    "setContext",
    "testbenchExtension.connectionActive",
    connection !== null,
  );
