...
export class TestElementsDataProvider {
    ...
    private setupConfigurationChangeListener(): void {
        const configChangeDisposable = vscode.workspace.onDidChangeConfiguration((event) => {
            const resourceMarkerChanged = event.affectsConfiguration(
                `testbenchExtension.${ConfigKeys.TB2ROBOT_RESOURCE_MARKER}`
            );
        ...
        const recursiveFilter = (testElementData: TestElementData, inheritedMatch: boolean): TestElementData | null => {
            ...
            const shouldIncludeCurrentNode =
                visibilityMode === "allSubdivisions"
                    ? this.shouldIncludeInAllSubdivisionsMode(testElementData, validChildren.length)
    ...
    /**
     * Filtering behavior used by the allSubdivisions mode.
     * Keeps subdivisions and keywords visible and suppresses unknown leaf types.
     */
    private shouldIncludeInAllSubdivisionsMode(testElementData: TestElementData, validChildrenCount: number): boolean {
        if (
            testElementData.testElementType === TestElementType.Subdivision ||
            testElementData.testElementType === TestElementType.Keyword
        ) {
            return true;
        }

        return validChildrenCount > 0;
    }
