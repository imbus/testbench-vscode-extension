...
export class TestBenchTreeDataProvider implements vscode.TreeDataProvider<TreeItem> {
    ...
    async getChildren(element?: TreeItem): Promise<TreeItem[]> {
        ...
        if (!element) {
            ...
            return projects.map(
                (project) => new TreeItem(project.name, "project", vscode.TreeItemCollapsibleState.Collapsed, project)
            );
        } else if (element.contextValue === "project") {
            // Get TOVs for the selected project
            const tovItems = element.item.testObjectVersions || [];
            return tovItems.map(
                (tov: { name: string }) => new TreeItem(tov.name, "tov", vscode.TreeItemCollapsibleState.Collapsed, tov)
            );
        } else if (element.contextValue === "tov") {
            // Get test cycles for the selected TOV
            const cycleItems = element.item.testCycles || [];
            return cycleItems.map(
                (cycle: { name: string }) =>
                    new TreeItem(cycle.name, "cycle", vscode.TreeItemCollapsibleState.None, cycle)
            );
        }
        return [];
    }
